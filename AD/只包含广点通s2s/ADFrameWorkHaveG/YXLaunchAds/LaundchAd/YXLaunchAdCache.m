...
@implementation YXLaunchAdCache
...
+(BOOL)saveImageData:(NSData *)data imageURL:(NSURL *)url{
    NSString *path = [NSString stringWithFormat:@"%@/%@",[self YXLaunchAdCachePath],[self keyWithURL:url]];
    if (data) {
        BOOL result = [[NSFileManager defaultManager] createFileAtPath:path contents:data attributes:nil];
        if (!result) YXLaunchAdLog(@"cache file error for URL: %@", url);
        return result;
    }
    return NO;
}
...
+(BOOL)saveVideoAtLocation:(NSURL *)location URL:(NSURL *)url{
    NSString *savePath = [[self YXLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
...
+ (NSString *)YXLaunchAdCachePath{
    NSString *path =[NSHomeDirectory() stringByAppendingPathComponent:@"Library/YXLaunchAdCache"];
...
+(NSString *)imagePathWithURL:(NSURL *)url{
    if(url==nil) return nil;
    return [[self YXLaunchAdCachePath] stringByAppendingPathComponent:[self keyWithURL:url]];
}

+(NSString *)videoPathWithURL:(NSURL *)url{
    if(url==nil) return nil;
    return [[self YXLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
}

+(NSString *)videoPathWithFileName:(NSString *)videoFileName{
    if(videoFileName.length==0) return @"";
    return [[self YXLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:[NSURL URLWithString:videoFileName]]];
}
...
+(void)clearDiskCache{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *path = [self YXLaunchAdCachePath];
        [fileManager removeItemAtPath:path error:nil];
        [self checkDirectory:[self YXLaunchAdCachePath]];
    });
}
...
+(void)clearDiskCacheExceptImageUrlArray:(NSArray<NSURL *> *)exceptImageUrlArray{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *allFilePaths = [self allFilePathWithDirectoryPath:[self YXLaunchAdCachePath]];
        ...
        YXLaunchAdLog(@"allFilePath = %@",allFilePaths);
    });
}
...
+(float)diskCacheSize{
    NSString *directoryPath = [self YXLaunchAdCachePath];
    BOOL isDir = NO;
...
+ (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        YXLaunchAdLog(@"create cache directory failed, error = %@", error);
    } else {
        [self addDoNotBackupAttribute:path];
    }
//    YXLaunchAdLog(@"YXLaunchAdCachePath = %@",path);
}
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        YXLaunchAdLog(@"error to set do not backup attribute, error = %@", error);
    }
}
