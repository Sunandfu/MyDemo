...
const NSTimeInterval kYXFLAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `YXFLAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, YXFLAnimatedImageDataSizeCategory) {
    YXFLAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    YXFLAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    YXFLAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    YXFLAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, YXFLAnimatedImageFrameCacheSize) {
    YXFLAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    YXFLAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    YXFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    YXFLAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol YXFLAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(YXFLAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(YXFLAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(YXFLAnimatedImage *)animatedImage;
@end
#endif


@interface YXFLAnimatedImage ()
...
// The actual type of the object is `YXFLWeakProxy`.
@property (nonatomic, strong, readonly) YXFLAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<YXFLAnimatedImageDebugDelegate> debug_delegate;
#endif
...
static NSHashTable *yxAllAnimatedImagesWeak;

@implementation YXFLAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > YXFLAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > YXFLAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [YXFLAnimatedImage class]) {
        // UIKit memory warning notification handler shared by all of the instances
        yxAllAnimatedImagesWeak = [NSHashTable weakObjectsHashTable];
        
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            // UIKit notifications are posted on the main thread. didReceiveMemoryWarning: is expecting the main run loop, and we don't lock on yxAllAnimatedImagesWeak
            ...
            // Note that YXFLAnimatedImages can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
            @synchronized(yxAllAnimatedImagesWeak) {
                images = [[yxAllAnimatedImagesWeak allObjects] copy];
            }
...
- (instancetype)init
{
    YXFLAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(YXFLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `YXFLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (!hasData) {
        FLLog(YXFLLogLevelError, @"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            FLLog(YXFLLogLevelError, @"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            FLLog(YXFLLogLevelError, @"Supplied data is of type %@ and doesn't seem to be GIF data %@", imageSourceContainerType, data);
            return nil;
        }
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        // If we don't get a delay time from the properties, fall back to `yxkDelayTimeIntervalDefault` or carry over the preceding frame's value.
                        const NSTimeInterval yxkDelayTimeIntervalDefault = 0.1;
                        if (!delayTime) {
                            if (i == 0) {
                                FLLog(YXFLLogLevelInfo, @"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                                delayTime = @(yxkDelayTimeIntervalDefault);
                            } else {
                                FLLog(YXFLLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                                delayTime = delayTimesForIndexesMutable[@(i - 1)];
                            }
                        }
                        // Support frame delays as low as `kYXFLAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `yxkDelayTimeIntervalDefault` for legacy compatibility.
                        ...
                        if ([delayTime floatValue] < ((float)kYXFLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(YXFLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], yxkDelayTimeIntervalDefault, kYXFLAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(yxkDelayTimeIntervalDefault);
                        }
                        delayTimesForIndexesMutable[@(i)] = delayTime;
                    } else {
                        skippedFrameCount++;
                        FLLog(YXFLLogLevelInfo, @"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
                    }
                    CFRelease(frameImageRef);
                } else {
                    skippedFrameCount++;
                    FLLog(YXFLLogLevelInfo, @"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, self->_imageSource);
                }
            }
        }
        ...
        if (self.frameCount == 0) {
            FLLog(YXFLLogLevelInfo, @"Failed to create any valid frames for GIF with properties %@", imageProperties);
            return nil;
        } else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            FLLog(YXFLLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        } else {
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= YXFLAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= YXFLAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = YXFLAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = YXFLAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
        ...
        _weakProxy = (id)[YXFLWeakProxy weakProxyForObject:self];
        ...
        // Note that YXFLAnimatedImages can be created on any thread, so the hash table must be locked.
        @synchronized(yxAllAnimatedImagesWeak) {
            [yxAllAnimatedImagesWeak addObject:self];
        }
    }
    return self;
}
...
{
    YXFLAnimatedImage *animatedImage = [[YXFLAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index
{
    ...
    if (index >= self.frameCount) {
        FLLog(YXFLLogLevelWarn, @"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
        return nil;
    }
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    if (firstRange.length + secondRange.length != self.frameCount) {
        FLLog(YXFLLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    }
    ...
    YXFLAnimatedImage * __weak weakSelf = self;
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                FLLog(YXFLLogLevelVerbose, @"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i, (predrawDuration + slowdownDuration) * 1000, self);
#endif
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[YXFLAnimatedImage class]]) {
        YXFLAnimatedImage *animatedImage = (YXFLAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(YXFLLogLevelError, @"`image` isn't of expected types `UIImage` or `YXFLAnimatedImage`: %@", image);
    }
...
- (NSMutableIndexSet *)frameIndexesToCache
{
    ...
    if (self.frameCacheSizeCurrent == self.frameCount) {
        ...
        if ([indexesToCache count] != self.frameCacheSizeCurrent) {
            FLLog(YXFLLogLevelWarn, @"Number of frames to cache doesn't equal expected cache size.");
        }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize
{
    ...
    FLLog(YXFLLogLevelDebug, @"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
    
    // Schedule resetting the frame cache size max completely after a while.
    const NSTimeInterval yxkResetDelay = 3.0;
    [self.weakProxy performSelector:@selector(resetFrameCacheSizeMaxInternal) withObject:nil afterDelay:yxkResetDelay];
}


- (void)resetFrameCacheSizeMaxInternal
{
    self.frameCacheSizeMaxInternal = YXFLAnimatedImageFrameCacheSizeNoLimit;
    FLLog(YXFLLogLevelDebug, @"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(YXFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    FLLog(YXFLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)YXFLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = YXFLAnimatedImageFrameCacheSizeLowMemory;
    ...
    //
    const NSUInteger yxkGrowAttemptsMax = 2;
    const NSTimeInterval yxkGrowDelay = 2.0;
    if ((self.memoryWarningCount - 1) <= yxkGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(YXFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:yxkGrowDelay];
    }
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw
{
    ...
    if (!colorSpaceDeviceRGBRef) {
        FLLog(YXFLLogLevelError, @"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
    ...
    // Early return on failure!
    if (!bitmapContextRef) {
        FLLog(YXFLLogLevelError, @"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: %zu height: %zu bitsPerComponent: %zu bytesPerRow: %zu) for image %@", colorSpaceDeviceRGBRef, width, height, bitsPerComponent, bytesPerRow, imageToPredraw);
        return imageToPredraw;
    }
    ...
    // Early return on failure!
    if (!predrawnImage) {
        FLLog(YXFLLogLevelError, @"Failed to `imageWithCGImage:scale:orientation:` with image ref %@ created with color space %@ and bitmap context %@ and properties and properties (scale: %f orientation: %ld) for image %@", predrawnImageRef, colorSpaceDeviceRGBRef, bitmapContextRef, imageToPredraw.scale, (long)imageToPredraw.imageOrientation, imageToPredraw);
        return imageToPredraw;
    }
...
@implementation YXFLAnimatedImage (Logging)

static void (^_logBlock)(NSString *logString, YXFLLogLevel logLevel) = nil;
static YXFLLogLevel _logLevel;

+ (void)setLogBlock:(void (^)(NSString *logString, YXFLLogLevel logLevel))logBlock logLevel:(YXFLLogLevel)logLevel
{
    _logBlock = logBlock;
    _logLevel = logLevel;
}

+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(YXFLLogLevel)level
{
...
#pragma mark - YXFLWeakProxy

@interface YXFLWeakProxy ()
...
@implementation YXFLWeakProxy

#pragma mark Life Cycle

// This is the designated creation method of an `YXFLWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject
{
    YXFLWeakProxy *weakProxy = [YXFLWeakProxy alloc];
