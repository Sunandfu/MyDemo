...
/**
 ...
 @param frame 广告的frame
 @param adType 广告类型
 @param delegate 广告请求结果回调代理
 @param mediaId 媒体位Id
 @return 广告层所在的父视图View
 */
-(instancetype _Nullable ) initWithFrame:(CGRect)frame adType:(YXADTYPE)adType delegate:(id <YXBannerAdManagerDelegate> _Nullable) delegate  mediaId:(NSString*)mediaId BannerLocation:(BannerLocationType)bannerType;
