...
@interface YXInterstitialAdManager ()<BUInterstitialAdDelegate,UIWebViewDelegate,UIGestureRecognizerDelegate,YXWebViewDelegate>
{
    NSDictionary*_gdtAD;
...
- (void)initS2S
{
    ...
    [[Network sharedInstance] prepareDataAndRequestWithadkeyString:_mediaId width:self.frame.size.width height:self.frame.size.height macID:macId uid:[NetTool getOpenUDID]];
    [self initXAD];
}
- (void)initXAD
{
