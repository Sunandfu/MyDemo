    ...
    switch (sender.tag) {
        ...
        case 203:
        {
            ...
            if (self.model.data.asset.balance<self.model.data.rate) {
                ...
                message = [NSString stringWithFormat:@"当前金币可以兑换%d张一元乘车券，是否全部兑换？",self.model.data.asset.balance/self.model.data.rate];
            }
...
- (void)showAlertImageView{
    ...
    desc.text = [NSString stringWithFormat:@"获得%d张一元乘车券",self.model.data.asset.balance/self.model.data.rate];
    ...
    tips.text = [NSString stringWithFormat:@"您已拥有%d张一元乘车券",self.model.data.asset.voucher_num+self.model.data.asset.balance/self.model.data.rate];
