...
extern const NSTimeInterval kYXFLAnimatedImageDelayTimeIntervalMinimum;

//
//  An `YXFLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `YXFLAnimatedImageView`.
...
//
@interface YXFLAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `YXFLAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `YXFLAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
typedef NS_ENUM(NSUInteger, YXFLLogLevel) {
    YXFLLogLevelNone = 0,
    YXFLLogLevelError,
    YXFLLogLevelWarn,
    YXFLLogLevelInfo,
...
@interface YXFLAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [YXFLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface YXFLWeakProxy : NSProxy
