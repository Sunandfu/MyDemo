...
@protocol YXFLAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(YXFLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface YXFLAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<YXFLAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation YXFLAnimatedImageView
...
- (void)setAnimatedImage:(YXFLAnimatedImage *)animatedImage
{
...
{
    // Presision is set to half of the `kYXFLAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval yxkGreatestCommonDivisorPrecision = 2.0 / kYXFLAnimatedImageDelayTimeIntervalMinimum;
    ...
    for (NSNumber *value in delays) {
        scaledGCD = gcd(lrint([value floatValue] * yxkGreatestCommonDivisorPrecision), scaledGCD);
    }
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            YXFLWeakProxy *weakProxy = [YXFLWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    // Early return!
    if (!self.shouldAnimate) {
        FLLog(YXFLLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber) {
        ...
        if (image) {
            FLLog(YXFLLogLevelVerbose, @"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
            ...
            FLLog(YXFLLogLevelDebug, @"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
