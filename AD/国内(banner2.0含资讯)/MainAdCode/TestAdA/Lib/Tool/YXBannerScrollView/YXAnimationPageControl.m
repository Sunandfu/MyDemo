...
@interface YXAnimationPageControl()
...
+ (instancetype)initWithinitWithFrame:(CGRect)frame pageCount:(NSInteger)pages currentPageImage:(UIImage *)currentPageImage normalPageImage:(UIImage *)normalPageImage {
    YXAnimationPageControl *pageControl = [[self alloc] initWithFrame:frame];
...
- (void)_initSubViews {
    ...
    _startPage = 0;
    _dotAlpha = 0.5;
    _animationType = YXAnimationPageControlNoamal;
}
...
- (void)dealloc {
    NSLog(@"dealloc--YXHorizontalPageControl");
}
