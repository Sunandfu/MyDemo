...
- (void)setLaunch
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(backClicked) name:@"SFSafariDoneClick" object:nil];
}
...
- (void)splashAdDidLoad:(BUSplashAdView *)splashAd
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(didLoadAd:)]){
            if (self->_adType == YXScreenType) {
                [self.delegate didLoadAd:nil];
            }
        }
    });
...
- (void)splashAdDidClick:(BUSplashAdView *)splashAd
{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(didClickedAdWithUrlStr:)]){
...
-(void)YXLaunchAd:(YXLaunchAd *)launchAd clickAndOpenModel:(id)openModel clickPoint:(CGPoint)clickPoint{
    ...
    if ([dict[@"lanigirOaideMsi"] isEqualToString:@"1"]) {
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(didClickedAdWithUrlStr:)]){
            [self.delegate didClickedAdWithUrlStr:urlStr];
        }
