...
@interface YXLaunchAdDownload()

@property (strong, nonatomic) NSURLSession *session;
@property(strong,nonatomic)NSURLSessionDownloadTask *downloadTask;
@property (nonatomic, assign) unsigned long long totalLength;
@property (nonatomic, assign) unsigned long long currentLength;
@property (nonatomic, copy) YXLaunchAdDownloadProgressBlock progressBlock;
@property (strong, nonatomic) NSURL *url;

@end
@implementation YXLaunchAdDownload

@end

#pragma mark -  YXLaunchAdImageDownload
@interface YXLaunchAdImageDownload()<NSURLSessionDownloadDelegate,NSURLSessionTaskDelegate>

@property (nonatomic, copy ) YXLaunchAdDownloadImageCompletedBlock completedBlock;

@end
@implementation YXLaunchAdImageDownload

-(nonnull instancetype)initWithURL:(nonnull NSURL *)url delegateQueue:(nonnull NSOperationQueue *)queue progress:(nullable YXLaunchAdDownloadProgressBlock)progressBlock completed:(nullable YXLaunchAdDownloadImageCompletedBlock)completedBlock{
    self = [super init];
    if (self) {
        self.url = url;
...
#pragma makr - YXLaunchAdVideoDownload
@interface YXLaunchAdVideoDownload()<NSURLSessionDownloadDelegate,NSURLSessionTaskDelegate>

@property (nonatomic, copy ) YXLaunchAdDownloadVideoCompletedBlock completedBlock;
@end
@implementation YXLaunchAdVideoDownload

-(nonnull instancetype)initWithURL:(nonnull NSURL *)url delegateQueue:(nonnull NSOperationQueue *)queue progress:(nullable YXLaunchAdDownloadProgressBlock)progressBlock completed:(nullable YXLaunchAdDownloadVideoCompletedBlock)completedBlock{
    self = [super init];
    if (self) {
        self.url = url;
        self.progressBlock = progressBlock;
...
didFinishDownloadingToURL:(NSURL *)location {
    NSError *error=nil;
    NSURL *toURL = [NSURL fileURLWithPath:[YXLaunchAdCache videoPathWithURL:self.url]];
    [[NSFileManager defaultManager] copyItemAtURL:location toURL:toURL error:&error];
    if(error)  YXLaunchAdLog(@"error = %@",error);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_completedBlock) {
            if(!error){
...
@interface YXLaunchAdDownloader()<YXLaunchAdDownloadDelegate>
...
@implementation YXLaunchAdDownloader

+(nonnull instancetype )sharedDownloader{
    static YXLaunchAdDownloader *instance = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken,^{
        instance = [[YXLaunchAdDownloader alloc] init];
    });
    return instance;
}
...
- (instancetype)init{
    ...
    if (self) {
        ...
        _downloadImageQueue.name = @"com.it7090.YXLaunchAdDownloadImageQueue";
        _downloadVideoQueue = [NSOperationQueue new];
        _downloadVideoQueue.maxConcurrentOperationCount = 3;
        _downloadVideoQueue.name = @"com.it7090.YXLaunchAdDownloadVideoQueue";
        YXLaunchAdLog(@"YXLaunchAdCachePath:%@",[YXLaunchAdCache YXLaunchAdCachePath]);
    }
    return self;
}
...
- (void)downloadImageWithURL:(nonnull NSURL *)url progress:(nullable YXLaunchAdDownloadProgressBlock)progressBlock completed:(nullable YXLaunchAdDownloadImageCompletedBlock)completedBlock{
    ...
    YXLaunchAdImageDownload * download = [[YXLaunchAdImageDownload alloc] initWithURL:url delegateQueue:_downloadImageQueue progress:progressBlock completed:completedBlock];
...
- (void)downloadImageAndCacheWithURL:(nonnull NSURL *)url completed:(void(^)(BOOL result))completedBlock{
    ...
    [self downloadImageWithURL:url progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error) {
        if(error){
            ...
            [YXLaunchAdCache async_saveImageData:data imageURL:url completed:^(BOOL result, NSURL * _Nonnull URL) {
...
- (void)downLoadImageAndCacheWithURLArray:(nonnull NSArray <NSURL *> * )urlArray completed:(nullable YXLaunchAdBatchDownLoadAndCacheCompletedBlock)completedBlock{
    ...
    [urlArray enumerateObjectsUsingBlock:^(NSURL *url, NSUInteger idx, BOOL *stop) {
        if(![YXLaunchAdCache checkImageInCacheWithURL:url]){
...
- (void)downloadVideoWithURL:(nonnull NSURL *)url progress:(nullable YXLaunchAdDownloadProgressBlock)progressBlock completed:(nullable YXLaunchAdDownloadVideoCompletedBlock)completedBlock{
    NSString *key = [self keyWithURL:url];
    if(self.allDownloadDict[key]) return;
    YXLaunchAdVideoDownload * download = [[YXLaunchAdVideoDownload alloc] initWithURL:url delegateQueue:_downloadVideoQueue progress:progressBlock completed:completedBlock];
    download.delegate = self;
    [self.allDownloadDict setObject:download forKey:key];
}
...
- (void)downloadVideoAndCacheWithURL:(nonnull NSURL *)url completed:(void(^)(BOOL result))completedBlock{
    ...
    [self downloadVideoWithURL:url progress:nil completed:^(NSURL * _Nullable location, NSError * _Nullable error) {
        if(error){
            ...
            [YXLaunchAdCache async_saveVideoAtLocation:location URL:url completed:^(BOOL result, NSURL * _Nonnull URL) {
                if(completedBlock) completedBlock(result);
            }];
        }
    }];
...
- (void)downLoadVideoAndCacheWithURLArray:(nonnull NSArray <NSURL *> * )urlArray completed:(nullable YXLaunchAdBatchDownLoadAndCacheCompletedBlock)completedBlock{
    ...
    dispatch_group_t downLoadGroup = dispatch_group_create();
    [urlArray enumerateObjectsUsingBlock:^(NSURL *url, NSUInteger idx, BOOL *stop) {
        if(![YXLaunchAdCache checkVideoInCacheWithURL:url]){
             dispatch_group_enter(downLoadGroup);
...
-(NSString *)keyWithURL:(NSURL *)url{
    return [YXLaunchAdCache md5String:url.absoluteString];
}
