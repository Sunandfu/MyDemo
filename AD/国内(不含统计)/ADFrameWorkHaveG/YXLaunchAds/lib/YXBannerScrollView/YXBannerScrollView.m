...
@property (nonatomic, weak) YXHorizontalPageControl *horizontalPageControl;
@property (nonatomic, weak) YXAnimationPageControl  *gAnimationPageControl;
@property (nonatomic, strong) UIImageView *backgroundImageView; // 当imageURLs为空时的背景图

@end

@implementation YXBannerScrollView
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame imageNamesGroup:(NSArray *)imageNamesGroup pageControlStyle:(YXBannerScrollViewPageContolStyle)pageControlStyle {
    YXBannerScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame delegate:(id<YXBannerScrollViewDelegate>)delegate placeholderImage:(UIImage *)placeholderImage {
    YXBannerScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)setDelegate:(id<YXBannerScrollViewDelegate>)delegate {
    _delegate = delegate;
...
- (void)setPageControlDotSize:(CGSize)pageControlDotSize {
    ...
    if ([self.pageControl isKindOfClass:[YXPageControl class]]) {
        YXPageControl *pageContol = (YXPageControl *)_pageControl;
...
- (void)setCurrentPageDotColor:(UIColor *)currentPageDotColor {
    ...
    if ([self.pageControl isKindOfClass:[YXPageControl class]]) {
        YXPageControl *pageControl = (YXPageControl *)_pageControl;
        pageControl.dotColor = currentPageDotColor;
    } else if ([self.pageControl isKindOfClass:[YXHorizontalPageControl class]]) {
        YXHorizontalPageControl *pageControl = (YXHorizontalPageControl *)_pageControl;
        pageControl.currentPageColor = currentPageDotColor;
    }else if ([self.pageControl isKindOfClass:[YXAnimationPageControl class]]) {
...
- (void)setupPageControl {
    ...
    switch (self.pageControlStyle) {
        ...
        case YXBannerScrollViewPageControlHorizontal: {
            ...
            YXHorizontalPageControl *pageControl = [[YXHorizontalPageControl alloc] init];
        ...
        case YXBannerScrollViewPageImageRotation: {
            ...
            pageControl.animationType = YXAnimationPageControlRotation;
        ...
        case YXBannerScrollViewPageImageJump: {
            ...
            pageControl.pages = self.imagePathsGroup.count;
            pageControl.normalPageImage = self.pageDotImage;
            pageControl.animationType = YXAnimationPageControlJump;
            pageControl.currentPageImage = self.currentPageDotImage;
            pageControl.userInteractionEnabled = NO;
            pageControl.startPage = indexOnPageControl;
            [self addSubview:pageControl];
        ...
        case YXBannerScrollViewPageImageAnimated: {
            ...
            pageControl.pages = self.imagePathsGroup.count;
            pageControl.normalPageImage = self.pageDotImage;
            pageControl.animationType = YXAnimationPageControlNoamal;
            pageControl.currentPageImage = self.currentPageDotImage;
            pageControl.userInteractionEnabled = NO;
            pageControl.startPage = indexOnPageControl;
            [self addSubview:pageControl];
...
- (int)currentIndex {
    if (_mainView.YX_width == 0 || _mainView.YX_height == 0) {
        return 0;
    }
    int index = 0;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(registerAdViewForInteraction:didScrollToIndex:)]) {
        [self.delegate registerAdViewForInteraction:cell didScrollToIndex:itemIndex];
    }
    NSString *imagePath = self.imagePathsGroup[itemIndex];
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            //此接口为sd_setImageWithURL 留用
            [NetTool setImage:cell.imageView WithURLStr:imagePath placeholderImage:self.placeholderImage];
