...
- (void)showGif
{
    ...
    if(urlstr && ![urlstr isEqualToString:@""]){
        ...
        [YXImgUtil gifImgWithUrl:urlstr successBlock:^(NSData *data) {
            ...
            [weakSelf.webView loadData:data MIMEType:@"image/gif" textEncodingName:@"UTF-8" baseURL:[NSURL URLWithString:urlstr]];
            if (weakSelf.resultDict[@"logo_url"]) {
                NSString * logo_url = weakSelf.resultDict[@"logo_url"];
                UIImage *logoImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:logo_url]]];
                UIImageView *logoView = [[UIImageView alloc] initWithFrame:CGRectMake(weakSelf.webView.frame.size.width - 24 , weakSelf.webView.frame.size.height - 24, 24, 24)];
                logoView.image = logoImage ;
                [weakSelf.webView addSubview:logoView];
            }
            // 2.显示成功
            // 2.显示成功
            dispatch_async(dispatch_get_main_queue(), ^{
                if(weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(didLoadBannerAd)]){
                    [weakSelf.delegate didLoadBannerAd];
                }
                // 3.上报
