...
@interface SFChannelTags ()<UICollectionViewDelegate,UICollectionViewDataSource,SFChannelCellDeleteDelegate>{
...
- (void)makeTags{
    ...
    for (NSString *title in _myChannels) {
        SFChannel *mod = [[SFChannel alloc]init];
        mod.title = title;
...
- (void)setupViews{
    ...
    exit.frame = CGRectMake(15, SF_StatusBarHeight+10, 20, 20);
    [exit setImage:[UIImage imageNamed:@"XibAndPng.bundle/sf_Exit"] forState:UIControlStateNormal];
    ...
    [_mainView registerClass:[SFChannelCell class] forCellWithReuseIdentifier:@"cellIdentifier"];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    SFChannelCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:CellIdentifier forIndexPath:indexPath];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return CGSizeMake(SF_ScreenW/4-10, 53);
}
...
- (void)editTags:(UIButton *)sender{
    ...
    if (!_onEdit) {
        for (SFChannelCell *items in _mainView.visibleCells) {
...
- (BOOL)collectionView:(UICollectionView *)collectionView canMoveItemAtIndexPath:(NSIndexPath *)indexPath{
    SFChannelCell *cell = (SFChannelCell *)[collectionView cellForItemAtIndexPath:indexPath];
    if (indexPath.section == 0) {
        if (cell.model.resident) {
            return NO;
        }else{
...
-(void)collectionView:(UICollectionView *)collectionView moveItemAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath{
    SFChannel *object= _myTags[sourceIndexPath.item];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        if (_onEdit) {
            ...
            SFChannel *object = _myTags[indexPath.item];
            object.selected = YES;
...
- (void)deleteCellWithIndexPath:(NSIndexPath *)indexPath{
    SFChannelCell *cell = (SFChannelCell *)[_mainView cellForItemAtIndexPath:indexPath];
...
- (void)refreshDelBtnsTag{
    ...
    for (SFChannelCell *cell in _mainView.visibleCells) {
