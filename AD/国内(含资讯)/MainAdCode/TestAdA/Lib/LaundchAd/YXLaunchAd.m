...
- (YXLaunchAdButton *)skipButton
{
    if (!_skipButton) {
        ...
        if(!self.imageAdConfiguration.duration) self.imageAdConfiguration.duration = 8;
...
-(void)addSkipButtonForConfiguration:(YXLaunchAdConfiguration *)configuration{
    if(!configuration.duration) configuration.duration = 8;
...
-(void)startSkipDispathTimer{
    ...
    __block NSInteger duration = 8;//默认
