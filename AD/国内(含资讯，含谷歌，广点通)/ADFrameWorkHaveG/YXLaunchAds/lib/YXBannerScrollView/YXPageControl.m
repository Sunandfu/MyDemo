...
#import "YXAbstractDotView.h"
#import "YXAnimatedDotView.h"
#import "YXDotView.h"
 ...
 */
- (void)initialization {
    self.dotViewClass           = [YXAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(YXPageControl:didSelectPageAtIndex:)]) {
            [self.delegate YXPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView {
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[YXAnimatedDotView class]] && self.dotColor) {
            ((YXAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index {
    if (self.dotViewClass) {
        YXAbstractDotView *abstractDotView = (YXAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [YXAbstractDotView class]);
        }
