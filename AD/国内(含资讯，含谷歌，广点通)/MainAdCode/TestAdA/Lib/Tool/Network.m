...
+ (void)postUpdateDataWithURL:(NSString *)url parameters:(id)parameters{
...
+ (void)newsStatisticsWithType:(NSInteger)eventType NewsID:(NSString *)newsId CatID:(NSString *)catId lengthOfTime:(NSInteger)lengthOfTime{
    dispatch_group_t group = dispatch_group_create();
    dispatch_queue_t queue = dispatch_queue_create("com.yunxiang.newsStatics", DISPATCH_QUEUE_PRIORITY_DEFAULT);
    dispatch_group_async(group, queue, ^{
        ...
        NSString *lengthtime = [NSString stringWithFormat:@"%@",(eventType==4?@(lengthOfTime):@"")];
        NSString *url = [NSString stringWithFormat:@"https://nr.beilamusi.com/social/eventStatistic?userId=%@&mLocationId=%@&catId=%@&eventType=%@&newsId=%@&stayTime=%@",mediaId,mLocationId,catId,@(eventType),newsId,lengthtime];
        ...
        [parametDict setValue:@"1"                           forKey:@"osType"];
