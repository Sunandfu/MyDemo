...
-(void)tapImg:(UITapGestureRecognizer *)recognizer
{
    ...
    if ([dict[@"lanigirOaideMsi"] isEqualToString:@"1"]) {
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(didClickedMutBannerAdWithIndex:UrlStr:)]){
            [self.delegate didClickedMutBannerAdWithIndex:recognizer.view.tag UrlStr:urlStr];
        }
...
- (void)reloadDataScrollerView{
    ...
    self.bannerScrollView = [YXBannerScrollView cycleScrollViewWithFrame:self.bgHeaderView.bounds delegate:self placeholderImage:self.backgroundImage?self.backgroundImage:nil];
