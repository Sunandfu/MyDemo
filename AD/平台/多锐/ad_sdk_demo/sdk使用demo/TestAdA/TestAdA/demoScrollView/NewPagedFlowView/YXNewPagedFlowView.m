...
@implementation YXNewPagedFlowView
...
- (void)initialize{
    ...
    subviewClassName = @"YXPGIndexBannerSubiew";
...
- (void)autoNextPage {
    ...
    switch (self.orientation) {
        case YXNewPagedFlowViewOrientationHorizontal:{
            
            [_scrollView setContentOffset:CGPointMake(self.page * _pageSize.width, 0) animated:YES];
            break;
        }
        case YXNewPagedFlowViewOrientationVertical:{
...
- (void)queueReusableCell:(YXPGIndexBannerSubiew *)cell{
    [_reusableCells addObject:cell];
}

- (void)removeCellAtIndex:(NSInteger)index{
    YXPGIndexBannerSubiew *cell = [_cells objectAtIndex:index];
    ...
    switch (self.orientation) {
        case YXNewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                YXPGIndexBannerSubiew *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    YXPGIndexBannerSubiew *cell = [_cells objectAtIndex:pageIndex];
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        cell.didSelectCellBlock = ^(NSInteger tag, YXPGIndexBannerSubiew *cell) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
#pragma mark YXNewPagedFlowView API
...
- (void)reloadData {
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:subviewClassName] || [view isKindOfClass:[YXPGIndexBannerSubiew class]]) {
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case YXNewPagedFlowViewOrientationHorizontal://横向
...
- (YXPGIndexBannerSubiew *)dequeueReusableCell{
    YXPGIndexBannerSubiew *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//点击了cell
- (void)singleCellTapAction:(NSInteger)selectTag withCell:(YXPGIndexBannerSubiew *)cell {
