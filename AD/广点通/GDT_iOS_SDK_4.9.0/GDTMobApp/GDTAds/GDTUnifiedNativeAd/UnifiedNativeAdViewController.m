...
- (IBAction)loadUnifiedNativeAd:(id)sender {
    NSString *placementId = self.placementTextField.text.length > 0? self.placementTextField
    .text:self.placementTextField.placeholder;
    self.unifiedNativeAd = [[GDTUnifiedNativeAd alloc] initWithAppId:kGDTMobSDKAppId placementId:placementId];
...
- (IBAction)loadUnifiedNativeAdInTableView:(id)sender {
    NSString *placementId = self.placementTextField.text.length > 0? self.placementTextField
    .text:self.placementTextField.placeholder;
    UnifiedNativeAdViewInTableViewViewController *unifiedNativeAdInTableVC = [[UnifiedNativeAdViewInTableViewViewController alloc] initWithPlacementId:placementId];
...
- (void)setupImageAdRelatedViews:(GDTUnifiedNativeAdDataObject *)dataObject
{
    ...
    /*广告Icon*/
    logoView.frame = CGRectMake(CGRectGetWidth(unifiedNativeAdView.frame) - 54, CGRectGetHeight(unifiedNativeAdView.frame) - 18, kGDTLogoImageViewDefaultWidth, kGDTLogoImageViewDefaultHeight);
    UIImageView *iconV = [[UIImageView alloc] initWithFrame:CGRectMake(10, 5, 60, 60)];
    NSURL *iconURL = [NSURL URLWithString:dataObject.iconUrl];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        NSData *iconData = [NSData dataWithContentsOfURL:iconURL];
