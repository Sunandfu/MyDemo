...
- (void)viewDidLoad {
    ...
    //检测用户手机设置中语言种类  手机默认使用第一种语言
    NSLog(@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"AppleLanguages"]);
    
}
- (IBAction)StartWJTouchID {
    [self.touchID startWJTouchIDWithMessage:WJNotice(@"这里可以自定义", @"The Custom Message") fallbackTitle:WJNotice(@"输入密码", @"Fallback Title") delegate:self];
}
/**
 *  TouchID验证成功
 *
