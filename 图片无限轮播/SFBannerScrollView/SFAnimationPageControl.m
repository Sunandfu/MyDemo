...
@interface SFAnimationPageControl()
...
@implementation SFAnimationPageControl
...
+ (instancetype)initWithinitWithFrame:(CGRect)frame pageCount:(NSInteger)pages currentPageImage:(UIImage *)currentPageImage normalPageImage:(UIImage *)normalPageImage {
    SFAnimationPageControl *pageControl = [[self alloc] initWithFrame:frame];
...
- (void)_initSubViews {
    ...
    _startPage = 0;
    _dotAlpha = 0.5;
    _animationType = SFAnimationPageControlNoamal;
}

- (void)setCurrentPage:(NSInteger)page {
    NSLog(@"%ld=%ld=%ld",(long)self.pages,(long)page,(long)self.lastPage);
    ...
    if ((0 == page) && (self.pages == self.lastPage+1)) {
        self.lastPage = page;
    ...
    } else if ((self.pages == page+1) && (self.lastPage==0)) {
        self.lastPage = page;
        [self.animationImageView.layer removeAllAnimations];
        [self startrRotationImageView:self.animationImageView duration:1.0 clockwise:NO];
        [UIView animateWithDuration:0.5 animations:^{
            self.animationImageView.frame = CGRectMake((- 0.5) * (dotNomalWidth + self.dotMargin), 0, dotBigWidth, dotBigHeight);
            self.animationImageView.alpha = 0.0;
        } completion:^(BOOL finished) {
...
- (void)dealloc {
//    SFLog(@"dealloc--YXHorizontalPageControl");
}
