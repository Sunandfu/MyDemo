...
#define YXMainWindow  [UIApplication sharedApplication].keyWindow
#define YX_SAFE_BLOCK(BlockName, ...) ({ !BlockName ? nil : BlockName(__VA_ARGS__); })

#pragma mark - /////////////
#pragma mark - private cell

@interface YXPopupMenuCell : UITableViewCell
@property (nonatomic, assign) BOOL isShowSeparator;
@property (nonatomic, strong) UIColor * separatorColor;
@end

@implementation YXPopupMenuCell
...
#pragma mark - publics
+ (YXPopupMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<YXPopupMenuDelegate>)delegate
{
    YXPopupMenu *popupMenu = [[YXPopupMenu alloc] init];
...
+ (YXPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<YXPopupMenuDelegate>)delegate
{
    CGRect absoluteRect = [view convertRect:view.bounds toView:YXMainWindow];
...
+ (YXPopupMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (YXPopupMenu * popupMenu))otherSetting
{
    YXPopupMenu *popupMenu = [[YXPopupMenu alloc] init];
    popupMenu.point = point;
    popupMenu.titles = titles;
    popupMenu.images = icons;
    popupMenu.itemWidth = itemWidth;
    YX_SAFE_BLOCK(otherSetting,popupMenu);
    [popupMenu show];
    return popupMenu;
}

+ (YXPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (YXPopupMenu * popupMenu))otherSetting
{
    CGRect absoluteRect = [view convertRect:view.bounds toView:YXMainWindow];
    CGPoint relyPoint = CGPointMake(absoluteRect.origin.x + absoluteRect.size.width / 2, absoluteRect.origin.y + absoluteRect.size.height);
    YXPopupMenu *popupMenu = [[YXPopupMenu alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    YXPopupMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[YXPopupMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
    ...
    if (_images.count >= indexPath.row + 1) {
        if ([_images[indexPath.row] hasPrefix:@"http"]) {
            [SFNetTool setImage:cell.imageView WithURLStr:_images[indexPath.row] placeholderImage:nil];
        }else if ([_images[indexPath.row] isKindOfClass:[NSString class]]) {
...
{
    if ([[self getLastVisibleCell] isKindOfClass:[YXPopupMenuCell class]]) {
        YXPopupMenuCell *cell = [self getLastVisibleCell];
...
- (YXPopupMenuCell *)getLastVisibleCell
{
...
- (void)show
{
    [YXMainWindow addSubview:_menuBackView];
    [YXMainWindow addSubview:self];
    if ([[self getLastVisibleCell] isKindOfClass:[YXPopupMenuCell class]]) {
        YXPopupMenuCell *cell = [self getLastVisibleCell];
        cell.isShowSeparator = NO;
    }
...
- (void)setDefaultSettings
{
    ...
    _type = YXPopupMenuTypeDefault;
    _arrowDirection = YXPopupMenuArrowDirectionTop;
    _priorityDirection = YXPopupMenuPriorityDirectionTop;
    _minSpace = 10.0;
    _maxVisibleCount = 5;
    _itemHeight = 44;
    _isCornerChanged = NO;
    _showMaskView = YES;
    _menuBackView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, YXScreenWidth, YXScreenHeight)];
...
- (void)setType:(YXPopupMenuType)type
{
    _type = type;
    switch (type) {
        case YXPopupMenuTypeDark:
        {
...
- (void)setArrowDirection:(YXPopupMenuArrowDirection)arrowDirection
{
...
- (void)setPriorityDirection:(YXPopupMenuPriorityDirection)priorityDirection
{
...
- (void)updateUI
{
    ...
    if (_priorityDirection == YXPopupMenuPriorityDirectionTop) {
        if (_point.y + height + _arrowHeight > YXScreenHeight - _minSpace) {
            _arrowDirection = YXPopupMenuArrowDirectionBottom;
            _isChangeDirection = YES;
        }else {
            _arrowDirection = YXPopupMenuArrowDirectionTop;
            _isChangeDirection = NO;
        }
    }else if (_priorityDirection == YXPopupMenuPriorityDirectionBottom) {
    ...
    }else if (_priorityDirection == YXPopupMenuPriorityDirectionLeft) {
        if (_point.x + _itemWidth + _arrowHeight > YXScreenWidth - _minSpace) {
            _arrowDirection = YXPopupMenuArrowDirectionRight;
            _isChangeDirection = YES;
        }else {
            _arrowDirection = YXPopupMenuArrowDirectionLeft;
            _isChangeDirection = NO;
        }
    }else if (_priorityDirection == YXPopupMenuPriorityDirectionRight) {
    ...
    [self setRelyRect];
    if (_arrowDirection == YXPopupMenuArrowDirectionTop) {
        CGFloat y = _isChangeDirection ? _point.y  : _point.y;
        if (_arrowPosition > _itemWidth / 2) {
            self.frame = CGRectMake(YXScreenWidth - _minSpace - _itemWidth, y, _itemWidth, height + _arrowHeight);
    ...
    }else if (_arrowDirection == YXPopupMenuArrowDirectionBottom) {
    ...
    }else if (_arrowDirection == YXPopupMenuArrowDirectionLeft) {
    ...
    }else if (_arrowDirection == YXPopupMenuArrowDirectionRight) {
    ...
    }else if (_arrowDirection == YXPopupMenuArrowDirectionNone) {
...
- (void)changeRectCorner
{
    ...
    if (_arrowDirection == YXPopupMenuArrowDirectionTop || _arrowDirection == YXPopupMenuArrowDirectionBottom) {
...
{
    if (_priorityDirection == YXPopupMenuPriorityDirectionNone) {
        return;
    }
    if (_arrowDirection == YXPopupMenuArrowDirectionTop || _arrowDirection == YXPopupMenuArrowDirectionBottom) {
        if (_point.x + _itemWidth / 2 > YXScreenWidth - _minSpace) {
            _arrowPosition = _itemWidth - (YXScreenWidth - _minSpace - _point.x);
...
- (void)drawRect:(CGRect)rect
{
    UIBezierPath *bezierPath = [YXPopupMenuPath yb_bezierPathWithRect:rect rectCorner:_rectCorner cornerRadius:_cornerRadius borderWidth:_borderWidth borderColor:_borderColor backgroundColor:_backColor arrowWidth:_arrowWidth arrowHeight:_arrowHeight arrowPosition:_arrowPosition arrowDirection:_arrowDirection];
    [bezierPath fill];
    [bezierPath stroke];
}
