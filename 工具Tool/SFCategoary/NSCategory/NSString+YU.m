...
{
     NSString *outputStr = [uStr stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet characterSetWithCharactersInString:@"+' '/?%#&"].invertedSet];
    return outputStr;
}
...
+(NSString*)adjustTime:(NSDate*)date
{
    ...
    NSDateComponents *_components = [cal components:(NSCalendarUnitEra|NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay) fromDate:[NSDate date]];
    NSDate *today = [cal dateFromComponents:_components];
    
    _components = [cal components:(NSCalendarUnitEra|NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay) fromDate:date];
    NSDate *otherDate = [cal dateFromComponents:_components];
    
    if([today isEqualToDate:otherDate]) {
        _components = [cal components:(NSCalendarUnitHour|NSCalendarUnitMinute) fromDate:date];
        ...
        if ([today isEqualToDate:otherDate]) {
            ...
            _components = [cal components:(NSCalendarUnitEra|NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay|NSCalendarUnitHour|NSCalendarUnitMinute) fromDate:date];
...
+(CGSize)LabSize:(UIFont*)Labfont labTex:(NSString*)Text{
    ...
    CGSize actualsize = [Text boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width, 10000) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
    actualsize.width += 2;
    return actualsize;
}
