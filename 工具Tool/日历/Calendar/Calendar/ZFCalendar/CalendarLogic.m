...
- (NSMutableArray *)calculateDaysInPreviousMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (long int i = daysCount - partialDaysCount + 1; i < daysCount + 1; ++i) {
...
-(void)LunarForSolarYear:(CalendarDayModel *)calendarDay{
    ...
    if ([solarYear_arr[1] isEqualToString:@"初一"]) {
        calendarDay.Chinese_calendar = [NSString stringWithFormat:@"%@月",solarYear_arr[0]];
    }
    ...
    //正月初一：春节
        calendarDay.Chinese_holiday = @"春节";
    ...
    //腊月二十四 小年
        calendarDay.Chinese_holiday = @"小年";
        
    }else if([solarYear_arr[0]isEqualToString:@"腊"] &&
             [solarYear_arr[1]isEqualToString:@"三十"]){
        
    //腊月三十（小月二十九）：除夕
        calendarDay.Chinese_holiday = @"除夕";
