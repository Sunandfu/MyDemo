...
- (BOOL)shouldDirectEdit:(ZLPhotoModel *)model
{
    ...
    if (self.configuration.maxEditEqualVideoTime) {
        if(round(model.asset.duration) < self.configuration.maxEditVideoTime){
            self.configuration.maxEditVideoTime = round(model.asset.duration);
        }
    }
    //当前点击视频可编辑
