...
- (void)resetBottomBtnsStatus:(BOOL)getBytes
{
    ...
    if (nav.arrSelectedModels.count == 1) {
        ...
        if (configuration.maxEditEqualVideoTime) {
            if(round(m.asset.duration) < configuration.maxEditVideoTime){
                configuration.maxEditVideoTime = round(m.asset.duration);
            }
        }
...
- (BOOL)shouldDirectEdit:(ZLPhotoModel *)model
{
    ...
    if (configuration.maxEditEqualVideoTime) {
        if(round(model.asset.duration) < configuration.maxEditVideoTime){
            configuration.maxEditVideoTime = round(model.asset.duration);
        }
    }
    //当前点击视频可编辑
