...
- (void)viewDidLoad {
    ...
    UIScrollView *scroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 360, self.view.bounds.size.width, self.view.bounds.size.height-360)];
    [self.view addSubview:scroll];
    scroll.contentSize = CGSizeMake(self.view.bounds.size.width, [UIFont familyNames].count*35);
    for (int i=0; i<[UIFont familyNames].count; i++) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, i*35, self.view.bounds.size.width, 35)];
        NSString *string = [NSString stringWithFormat:@"%@ 测试字体 123456",[UIFont familyNames][i]];
        label.text = string;
        label.font = [UIFont fontWithName:[UIFont familyNames][i] size:17];
