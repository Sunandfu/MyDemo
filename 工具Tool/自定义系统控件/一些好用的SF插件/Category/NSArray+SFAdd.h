...
@interface NSArray<ObjectType> (SFAdd)
...
/**
 ...
 @return 合并完的结果
 */
+ (instancetype)sf_arrayWithObjects:(ObjectType)object, ...;

/**
 *  将多维数组打平成一维数组再遍历所有子元素
 */
- (void)sf_enumerateNestedArrayWithBlock:(void (NS_NOESCAPE^)(id obj, BOOL *stop))block;
 ...
 */
- (NSArray<ObjectType> *)sf_filterWithBlock:(BOOL (NS_NOESCAPE^)(ObjectType item))block;

/**
*  转换数组元素，将每个 item 都经过 block 转换成一遍 返回转换后的新数组
*/
- (NSArray *)sf_mapWithBlock:(id (NS_NOESCAPE^)(ObjectType item))block;
