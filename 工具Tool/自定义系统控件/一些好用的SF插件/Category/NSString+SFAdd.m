...
- (NSArray<NSString *> *)sf_toTrimmedArray {
    return [[self sf_toArray] sf_filterWithBlock:^BOOL(NSString *item) {
        return item.sf_trim.length > 0;
    }];
}
...
- (NSString *)sf_trimAllWhiteSpace {
    return [self stringByReplacingOccurrencesOfString:@"\\s" withString:@"" options:NSRegularExpressionSearch range:NSMakeRange(0, self.length)];
}

- (NSString *)sf_trimLineBreakCharacter {
...
- (NSString *)sf_stringByEncodingUserInputQuery {
    return [self stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet sf_URLUserInputQueryAllowedCharacterSet]];
}
...
+ (NSString *)hexLetterStringWithInteger:(NSInteger)integer {
    NSAssert(integer < 16, @"NSString (SFAdd)", @"%s 参数仅接受小于16的值，当前传入的是 %@", __func__, @(integer));
...
- (NSUInteger)sf_lengthWhenCountingNonASCIICharacterAsTwo {
    NSUInteger length = 0;
...
- (NSString *)sf_substringAvoidBreakingUpCharacterSequencesFromIndex:(NSUInteger)index lessValue:(BOOL)lessValue countingNonASCIICharacterAsTwo:(BOOL)countingNonASCIICharacterAsTwo {
    NSUInteger length = countingNonASCIICharacterAsTwo ? self.sf_lengthWhenCountingNonASCIICharacterAsTwo : self.length;
    NSAssert(index < length, @"NSString (SFAdd)", @"%s, index out of bounds.", __func__);
...
- (NSString *)sf_substringAvoidBreakingUpCharacterSequencesFromIndex:(NSUInteger)index {
    return [self sf_substringAvoidBreakingUpCharacterSequencesFromIndex:index lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)sf_substringAvoidBreakingUpCharacterSequencesToIndex:(NSUInteger)index {
    return [self sf_substringAvoidBreakingUpCharacterSequencesToIndex:index lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)sf_substringAvoidBreakingUpCharacterSequencesWithRange:(NSRange)range {
    return [self sf_substringAvoidBreakingUpCharacterSequencesWithRange:range lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)sf_stringByRemoveLastCharacter {
    return [self sf_stringByRemoveCharacterAtIndex:self.length - 1];
}

- (NSString *)sf_stringMatchedByPattern:(NSString *)pattern {
    return [self sf_stringMatchedByPattern:pattern groupIndex:0];
}
...
- (NSString *)sf_stringMatchedByPattern:(NSString *)pattern groupName:(NSString *)name {
    if (@available(iOS 11.0, *)) {
        ...
        if (result.numberOfRanges > 1) {
            ...
            NSAssert(range.location != NSNotFound, @"NSString (SFAdd)", @"%s, 不存在名为 %@ 的 group name", __func__, name);
...
@implementation NSString (SFAdd_StringFormat)
...
+ (instancetype)sf_stringWithCGFloat:(CGFloat)floatValue {
    return [NSString sf_stringWithCGFloat:floatValue decimal:2];
}
