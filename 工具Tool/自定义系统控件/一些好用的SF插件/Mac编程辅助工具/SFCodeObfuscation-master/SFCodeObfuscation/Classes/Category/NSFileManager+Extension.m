...
{
    if (filepath.sf_stringByRemovingSpace.length == 0 || !completion) return;
...
{
    if (filename.sf_stringByRemovingSpace.length == 0 || !completion) return;
...
{
    if (dir.sf_stringByRemovingSpace.length == 0) return nil;
...
+ (NSString *)sf_checkPathExists:(NSString *)path
{
    ...
    while ([mgr fileExistsAtPath:filepath]) {
        [NSFileManager sf_divideFilename:filepath completion:^(NSString *filename, NSString *extension) {
