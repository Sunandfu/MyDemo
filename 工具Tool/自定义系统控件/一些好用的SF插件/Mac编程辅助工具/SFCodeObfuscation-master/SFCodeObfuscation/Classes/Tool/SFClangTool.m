...
/** 类名、方法名 */
@interface SFTokensClientData : NSObject
@property (nonatomic, strong) NSArray *prefixes;
@property (nonatomic, strong) NSMutableSet *tokens;
@property (nonatomic, copy) NSString *file;
@end

@implementation SFTokensClientData
@end

/** 字符串 */
@interface SFStringsClientData : NSObject
@property (nonatomic, strong) NSMutableSet *strings;
@property (nonatomic, copy) NSString *file;
@end

@implementation SFStringsClientData
@end

@implementation SFClangTool
...
enum CXChildVisitResult _visitTokens(CXCursor cursor,
    ...
    SFTokensClientData *data = (__bridge SFTokensClientData *)clientData;
...
enum CXChildVisitResult _visitStrings(CXCursor cursor,
    ...
    SFStringsClientData *data = (__bridge SFStringsClientData *)clientData;
    if (!_isFromFile(data.file.UTF8String, cursor)) return CXChildVisit_Continue;
...
{
    SFStringsClientData *data = [[SFStringsClientData alloc] init];
    data.file = file;
...
+ (NSSet *)classesAndMethodsWithFile:(NSString *)file
                          ...
                          searchPath:(NSString *)searchPath
{
    SFTokensClientData *data = [[SFTokensClientData alloc] init];
    data.file = file;
               ...
               clientData:(CXClientData)clientData
{
    ...
    if (searchPath.length) {
        subDirs = [NSFileManager sf_subdirsAtPath:searchPath];
