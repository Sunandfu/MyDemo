...
- (NSString *)sf_hmacStringUsingAlg:(CCHmacAlgorithm)alg withKey:(NSString *)key {
    size_t size;
    switch (alg) {
...
- (NSData *)sf_hmacDataUsingAlg:(CCHmacAlgorithm)alg withKey:(NSData *)key {
    size_t size;
    switch (alg) {
        case kCCHmacAlgMD5: size = CC_MD5_DIGEST_LENGTH; break;
        case kCCHmacAlgSHA1: size = CC_SHA1_DIGEST_LENGTH; break;
...
- (NSString *)sf_hmacMD5StringWithKey:(NSString *)key {
    return [self sf_hmacStringUsingAlg:kCCHmacAlgMD5 withKey:key];
}

- (NSData *)sf_hmacMD5DataWithKey:(NSData *)key {
    return [self sf_hmacDataUsingAlg:kCCHmacAlgMD5 withKey:key];
}

- (NSString *)sf_hmacSHA1StringWithKey:(NSString *)key {
    return [self sf_hmacStringUsingAlg:kCCHmacAlgSHA1 withKey:key];
}

- (NSData *)sf_hmacSHA1DataWithKey:(NSData *)key {
    return [self sf_hmacDataUsingAlg:kCCHmacAlgSHA1 withKey:key];
}

- (NSString *)sf_hmacSHA224StringWithKey:(NSString *)key {
    return [self sf_hmacStringUsingAlg:kCCHmacAlgSHA224 withKey:key];
}

- (NSData *)sf_hmacSHA224DataWithKey:(NSData *)key {
    return [self sf_hmacDataUsingAlg:kCCHmacAlgSHA224 withKey:key];
}

- (NSString *)sf_hmacSHA256StringWithKey:(NSString *)key {
    return [self sf_hmacStringUsingAlg:kCCHmacAlgSHA256 withKey:key];
}

- (NSData *)sf_hmacSHA256DataWithKey:(NSData *)key {
    return [self sf_hmacDataUsingAlg:kCCHmacAlgSHA256 withKey:key];
}

- (NSString *)sf_hmacSHA384StringWithKey:(NSString *)key {
    return [self sf_hmacStringUsingAlg:kCCHmacAlgSHA384 withKey:key];
}

- (NSData *)sf_hmacSHA384DataWithKey:(NSData *)key {
    return [self sf_hmacDataUsingAlg:kCCHmacAlgSHA384 withKey:key];
}

- (NSString *)sf_hmacSHA512StringWithKey:(NSString *)key {
    return [self sf_hmacStringUsingAlg:kCCHmacAlgSHA512 withKey:key];
}

- (NSData *)sf_hmacSHA512DataWithKey:(NSData *)key {
    return [self sf_hmacDataUsingAlg:kCCHmacAlgSHA512 withKey:key];
}
...
- (NSData *)sf_aes256EncryptWithKey:(NSData *)key iv:(NSData *)iv {
...
- (NSData *)sf_aes256DecryptWithkey:(NSData *)key iv:(NSData *)iv {
    if (key.length != 16 && key.length != 24 && key.length != 32) {
        return nil;
    }
    if (iv.length != 16 && iv.length != 0) {
...
+ (NSData *)sf_dataWithHexString:(NSString *)hexStr {
...
static const char sf_base64EncodingTable[64]
= "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
static const short sf_base64DecodingTable[256] = {
...
- (NSString *)sf_base64EncodedString {
    NSUInteger length = self.length;
    if (length == 0)
        return @"";
    ...
    for (i = 0; i < length; i += 3) {
        ...
        output[index + 0] = sf_base64EncodingTable[(value >> 18) & 0x3F];
        output[index + 1] = sf_base64EncodingTable[(value >> 12) & 0x3F];
        output[index + 2] = ((i + 1) < length)
        ? sf_base64EncodingTable[(value >> 6) & 0x3F]
        : '=';
        output[index + 3] = ((i + 2) < length)
        ? sf_base64EncodingTable[(value >> 0) & 0x3F]
        : '=';
    }
...
+ (NSData *)sf_dataWithBase64EncodedString:(NSString *)base64EncodedString {
    ...
    while (inputPoint < length) {
        ...
        output[outputPoint++] = (sf_base64DecodingTable[i0] << 2)
        | (sf_base64DecodingTable[i1] >> 4);
        if (outputPoint < outputLength) {
            output[outputPoint++] = ((sf_base64DecodingTable[i1] & 0xf) << 4)
            | (sf_base64DecodingTable[i2] >> 2);
        }
        if (outputPoint < outputLength) {
            output[outputPoint++] = ((sf_base64DecodingTable[i2] & 0x3) << 6)
            | sf_base64DecodingTable[i3];
        }
    }
...
+ (NSData *)sf_dataNamed:(NSString *)name {
