...
extern const NSTimeInterval kSFAnimatedImageDelayTimeIntervalMinimum;

//
//  An `SFAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `SFAnimatedImageView`.
...
//
@interface SFAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `SFAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `SFAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface SFAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [SFAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface SFWeakProxy : NSProxy
