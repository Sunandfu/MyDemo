...
const NSTimeInterval kSFAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `SFAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, SFAnimatedImageDataSizeCategory) {
    SFAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    SFAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    SFAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    SFAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, SFAnimatedImageFrameCacheSize) {
    SFAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    SFAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    SFAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    SFAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol SFAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(SFAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(SFAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(SFAnimatedImage *)animatedImage;
@end
#endif


@interface SFAnimatedImage ()
...
// The actual type of the object is `SFWeakProxy`.
@property (nonatomic, strong, readonly) SFAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<SFAnimatedImageDebugDelegate> debug_delegate;
#endif
...
static NSHashTable *SFAllAnimatedImagesWeak;

@implementation SFAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > SFAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > SFAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [SFAnimatedImage class]) {
        // UIKit memory warning notification handler shared by all of the instances
        SFAllAnimatedImagesWeak = [NSHashTable weakObjectsHashTable];
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            ...
            // Note that SFAnimatedImages can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
            @synchronized(SFAllAnimatedImagesWeak) {
                images = [[SFAllAnimatedImagesWeak allObjects] copy];
            }
...
- (instancetype)init
{
    SFAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(SFLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `SFAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (!hasData) {
        FLLog(SFLogLevelError, @"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            FLLog(SFLogLevelError, @"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            FLLog(SFLogLevelError, @"Supplied data is of type %@ and doesn't seem to be GIF data %@", imageSourceContainerType, data);
            return nil;
        }
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if (!delayTime.boolValue) {
                            if (i == 0) {
                                FLLog(SFLogLevelInfo, @"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                                delayTime = @(yxkDelayTimeIntervalDefault);
                            } else {
                                FLLog(SFLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                                delayTime = delayTimesForIndexesMutable[@(i - 1)];
                            }
                        }
                        // Support frame delays as low as `kSFAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `yxkDelayTimeIntervalDefault` for legacy compatibility.
                        ...
                        if ([delayTime floatValue] < ((float)kSFAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(SFLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], yxkDelayTimeIntervalDefault, kSFAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(yxkDelayTimeIntervalDefault);
                        }
                        delayTimesForIndexesMutable[@(i)] = delayTime;
                    } else {
                        skippedFrameCount++;
                        FLLog(SFLogLevelInfo, @"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
                    }
                    CFRelease(frameImageRef);
                } else {
                    skippedFrameCount++;
                    FLLog(SFLogLevelInfo, @"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, self->_imageSource);
                }
            }
        }
        ...
        if (self.frameCount == 0) {
            FLLog(SFLogLevelInfo, @"Failed to create any valid frames for GIF with properties %@", imageProperties);
            return nil;
        } else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            FLLog(SFLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        } else {
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= SFAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= SFAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = SFAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = SFAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
        ...
        _weakProxy = (id)[SFWeakProxy weakProxyForObject:self];
        ...
        // Note that SFAnimatedImages can be created on any thread, so the hash table must be locked.
        @synchronized(SFAllAnimatedImagesWeak) {
            [SFAllAnimatedImagesWeak addObject:self];
        }
    }
    return self;
}
...
{
    SFAnimatedImage *animatedImage = [[SFAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index
{
    ...
    if (index >= self.frameCount) {
        FLLog(SFLogLevelWarn, @"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
        return nil;
    }
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    if (firstRange.length + secondRange.length != self.frameCount) {
        FLLog(SFLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    }
    ...
    SFAnimatedImage * __weak weakSelf = self;
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                FLLog(SFLogLevelVerbose, @"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i, (predrawDuration + slowdownDuration) * 1000, self);
#endif
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[SFAnimatedImage class]]) {
        SFAnimatedImage *animatedImage = (SFAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(SFLogLevelError, @"`image` isn't of expected types `UIImage` or `SFAnimatedImage`: %@", image);
    }
...
- (NSMutableIndexSet *)frameIndexesToCache
{
    ...
    if (self.frameCacheSizeCurrent == self.frameCount) {
        ...
        if ([indexesToCache count] != self.frameCacheSizeCurrent) {
            FLLog(SFLogLevelWarn, @"Number of frames to cache doesn't equal expected cache size.");
        }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize
{
    ...
    FLLog(SFLogLevelDebug, @"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
...
{
    self.frameCacheSizeMaxInternal = SFAnimatedImageFrameCacheSizeNoLimit;
    FLLog(SFLogLevelDebug, @"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(SFAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    FLLog(SFLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)SFAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = SFAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= yxkGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(SFAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:yxkGrowDelay];
    }
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw
{
    ...
    if (!colorSpaceDeviceRGBRef) {
        FLLog(SFLogLevelError, @"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
    ...
    // Early return on failure!
    if (!bitmapContextRef) {
        FLLog(SFLogLevelError, @"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: %zu height: %zu bitsPerComponent: %zu bytesPerRow: %zu) for image %@", colorSpaceDeviceRGBRef, width, height, bitsPerComponent, bytesPerRow, imageToPredraw);
        return imageToPredraw;
    }
    ...
    // Early return on failure!
    if (!predrawnImage) {
        FLLog(SFLogLevelError, @"Failed to `imageWithCGImage:scale:orientation:` with image ref %@ created with color space %@ and bitmap context %@ and properties and properties (scale: %f orientation: %ld) for image %@", predrawnImageRef, colorSpaceDeviceRGBRef, bitmapContextRef, imageToPredraw.scale, (long)imageToPredraw.imageOrientation, imageToPredraw);
        return imageToPredraw;
    }
...
@implementation SFAnimatedImage (Logging)

static void (^sf_logBlock)(NSString *logString, SFLogLevel logLevel) = nil;
static SFLogLevel sf_logLevel;

+ (void)setLogBlock:(void (^)(NSString *logString, SFLogLevel logLevel))logBlock logLevel:(SFLogLevel)logLevel
{
    sf_logBlock = logBlock;
    sf_logLevel = logLevel;
}

+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(SFLogLevel)level
{
    if (level <= sf_logLevel && sf_logBlock && stringBlock) {
        sf_logBlock(stringBlock(), level);
    }
}
...
#pragma mark - SFWeakProxy

@interface SFWeakProxy ()
...
@implementation SFWeakProxy

#pragma mark Life Cycle

// This is the designated creation method of an `SFWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject
{
    SFWeakProxy *weakProxy = [SFWeakProxy alloc];
