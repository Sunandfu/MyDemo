...
#import "SFAnimatedImageView.h"
#import "SFAnimatedImage.h"
#import <QuartzCore/QuartzCore.h>


#if defined(DEBUG) && DEBUG
@protocol SFAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(SFAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface SFAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<SFAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation SFAnimatedImageView
...
- (void)setAnimatedImage:(SFAnimatedImage *)animatedImage
{
...
{
    // Presision is set to half of the `kSFAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval yxkGreatestCommonDivisorPrecision = 2.0 / kSFAnimatedImageDelayTimeIntervalMinimum;

    NSArray *delays = self.animatedImage.delayTimesForIndexes.allValues;

    // Scales the frame delays by `yxkGreatestCommonDivisorPrecision`
    // then converts it to an UInteger for in order to calculate the GCD.
    NSUInteger scaledGCD = lrint([delays.firstObject floatValue] * yxkGreatestCommonDivisorPrecision);
    for (NSNumber *value in delays) {
        scaledGCD = sf_gcd(lrint([value floatValue] * yxkGreatestCommonDivisorPrecision), scaledGCD);
    }

    // Reverse to scale to get the value back into seconds.
    return scaledGCD / yxkGreatestCommonDivisorPrecision;
}


static NSUInteger sf_gcd(NSUInteger a, NSUInteger b)
{
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            SFWeakProxy *weakProxy = [SFWeakProxy weakProxyForObject:self];
        ...
        const NSTimeInterval yxkDisplayRefreshRate = 60.0; // 60Hz
        self.displayLink.frameInterval = MAX([self frameDelayGreatestCommonDivisor] * yxkDisplayRefreshRate, 1);
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    // Early return!
    if (!self.shouldAnimate) {
        FLLog(SFLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber.boolValue) {
        ...
        if (image) {
            FLLog(SFLogLevelVerbose, @"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
            ...
            FLLog(SFLogLevelDebug, @"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
