...
@interface SFRequestTask ()<NSURLConnectionDataDelegate, NSURLSessionDataDelegate>
...
- (void)start {
    ...
    if (self.requestOffset > 0) {
        [request addValue:[NSString stringWithFormat:@"bytes=%ld-%ld", (long)self.requestOffset, (long)self.fileLength - 1] forHTTPHeaderField:@"Range"];
    }
...
//        NSLog(@"下载取消");
        ...
        if (error) {
            ...
            //可以缓存则保存文件
            if (self.cache) {
                [SFFileHandle cacheTempFileWithFileName:[NSString fileNameWithURL:self.requestURL]];
            }
