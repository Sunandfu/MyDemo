...
- (void)addButtonItem {
    if (self.alertViewBackgroundColor) {
        _alertView.backgroundColor = self.alertViewBackgroundColor;
    }
    if (self.alertViewTitleColor) {
        _labelTitle.textColor = self.alertViewTitleColor;
    }
    if (self.alertViewMessageColor) {
        _labelmessage.textColor = self.alertViewMessageColor;
    }
...
//        button.translatesAutoresizingMaskIntoConstraints = NO;
        ...
        if (self.lineColor) {
            button.layer.shadowColor = [self.lineColor CGColor];
        }
...
- (void)showAnimation {
    ...
    popAnimation.values = @[[NSValue valueWithCATransform3D:CATransform3DMakeScale(1.05f, 1.05f, 1.0f)],
