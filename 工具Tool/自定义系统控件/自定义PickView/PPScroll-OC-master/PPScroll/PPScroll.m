...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if (selected) {
        ...
        _label.textColor = _textColor_h;
    }else{
        _label.font = _textFont;
        _label.textColor = _textColor_n;
    }
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    float adjustedOffsetY = round(targetOffsetY/_bannerHeight)*_bannerHeight;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.label.text = [NSString stringWithFormat:@"T%ld  C%ld",(long)tableView.tag - 100,(long)indexPath.row];
