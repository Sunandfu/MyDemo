...
- (void)viewDidLoad {
    ...
    anno.coordinate = CLLocationCoordinate2DMake([WBRedEnvelopConfig sharedConfig].latitude, [WBRedEnvelopConfig sharedConfig].longitude);
    anno.title = [NSString stringWithFormat:@"经度：%f",[WBRedEnvelopConfig sharedConfig].longitude];
    anno.subtitle = [NSString stringWithFormat:@"纬度：%f",[WBRedEnvelopConfig sharedConfig].latitude];
    
    self.bottomView.longitudeLabel.text = [NSString stringWithFormat:@"经度：%f",[WBRedEnvelopConfig sharedConfig].longitude];
    self.bottomView.latitudeLabel.text = [NSString stringWithFormat:@"纬度：%f",[WBRedEnvelopConfig sharedConfig].latitude];
    //反地理编码
    _locManager = [[LocationConverter alloc] init];
    [_locManager reverseGeocodeWithlatitude:[WBRedEnvelopConfig sharedConfig].latitude longitude:[WBRedEnvelopConfig sharedConfig].longitude success:^(NSString *address) {
...
- (void)updateLocationWithCoordinate:(CLLocationCoordinate2D)coordinate {
    [WBRedEnvelopConfig sharedConfig].latitude = coordinate.latitude;
    [WBRedEnvelopConfig sharedConfig].longitude = coordinate.longitude;
...
- (void)locationSearchViewController:(UIViewController *)sender didSelectLocationWithName:(NSString *)name address:(NSString *)address mapItem:(MKMapItem *)mapItm {
    ...
    if (mapItm == nil) {
        coordinate = CLLocationCoordinate2DMake([WBRedEnvelopConfig sharedConfig].latitude, [WBRedEnvelopConfig sharedConfig].longitude);
    }
    else {
