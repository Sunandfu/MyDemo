...
- (instancetype)initWithSender:(UIViewController*)sender Button:(UIButton *)button{
    ...
    self = [super initWithFrame:frame];
    if (self) {
        [self buildViews:sender Button:button];
    }
    return self;
}
-(void)buildViews:(UIViewController*)sender Button:(UIButton *)button{
    _sender = sender;
    
    [button addTarget:self action:@selector(switchMenu) forControlEvents:UIControlEventTouchUpInside];
