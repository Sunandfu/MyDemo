...
- (void)commonInit
{
    ...
    if (self.horizontal==YES) {
        _dialView = [[TRSDialView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, contentHeight)WithIsFloatNumber1:self.typeOfNumber isHorizontal:YES];
    }else{
        _dialView = [[TRSDialView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, contentHeight)WithIsFloatNumber1:self.typeOfNumber isHorizontal:NO];
    }
    _dialView.isFloatNumber1 = self.typeOfNumber;
...
-(void)setNumberOfFloatWith:(BOOL)isFloatNumber
{
//    self.isFloatNumber = isFloatNumber;
//    NSLog(@"self.floatNumber fuzhi %d",[self class]);
}
- (id)initWithFrame:(CGRect)frame WithTypeOfNumber:(NSInteger)type isHorizontal:(BOOL)hvType
{
    
    self = [super initWithFrame:frame];
    self.typeOfNumber = type;
    if (self) {
        self.horizontal = hvType;
        [self commonInit];
        
    }
//    [self setNumberOfFloatWith:self.isFloatNumber];
//    NSLog(@"self.floatNumber%d",self.isFloatNumber);
    return self;
}
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if ([self.currentDelegate respondsToSelector:@selector(getCurrentValue)]) {
        [self.currentDelegate getCurrentValue];
    }
}
...
{
    return [NSString stringWithFormat:@"%ld",(long)self.currentValue];
}
