...
- (void)drawLabelWithContext:(CGContextRef)context
    ...
    [text drawInRect:CGRectMake(label_x, point.y + label_y_offset, boundingBox.width, boundingBox.height)withAttributes:att];
...
- (void)drawTicksWithContext:(CGContextRef)context atX:(int)x
{
    ...
    } else if ([self isMajorOrTick:x]){
        
        // Save the current context so we revert some of the changes laster
        CGContextSaveGState(context);
        if(!self.secondTickColor){
            self.secondTickColor = self.majorTickColor;
        }
        [self drawMajorTickWithContext:context
                               atPoint:point
                             withColor:self.secondTickColor
...
- (BOOL)isMajorOrTick:(int)y {
    
    int tick_number = (int)(y - self.leading) / self.minorTickDistance;
