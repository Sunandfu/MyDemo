...
@synthesize url, delegate, connection, imageData, userInfo, lowPriority, progressive, progressiveShow;
...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    // add by DJ
    if (self.progressiveShow && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:receivedSize:expectedSize:)])
    {
        const NSUInteger totalSize = [imageData length];
        
        if (totalSize <= expectedSize)
        {
            [delegate imageDownloader:self receivedSize:totalSize expectedSize:expectedSize];
        }
    }
    else if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)])
    {
