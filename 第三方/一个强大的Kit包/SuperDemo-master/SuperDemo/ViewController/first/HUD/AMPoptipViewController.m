...
- (IBAction)actionButton:(UIButton *)sender
{
    ...
    if (sender == self.buttonTopLeft) {
        ...
        [self.popTip showText:@"I'm a popover popping over-> AMPopTipDirectionDown" direction:AMPopTipDirectionDown maxWidth:200 inView:self.view fromFrame:sender.frame];
    }
    ...
    if (sender == self.buttonBottomLeft) {
        ...
        [self.popTip showText:@"I'm a popover popping over -> AMPopTipDirectionUp" direction:AMPopTipDirectionUp maxWidth:200 inView:self.view fromFrame:sender.frame];
    }
    if (sender == self.buttonBottomRight) {
        self.popTip.popoverColor = [UIColor colorWithRed:0.81 green:0.04 blue:0.14 alpha:1];
        NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:@"I'm a popover popping over -> AMPopTipDirectionUp" attributes:@{NSFontAttributeName: [UIFont fontWithName:@"Avenir-Medium" size:12], NSForegroundColorAttributeName: [UIColor whiteColor]}];
