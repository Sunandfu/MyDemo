...
@implementation Demo6Transition
...
- (void)setupTransition
{
	...
	vertices[6] = 2.0/TOTAL1;
	vertices[7] = -1.5;
	vertices[8] = 0.0;
	
	vertices[9] = 2.0/TOTAL1;
	vertices[10] = 1.5;
	vertices[11] = 0.0;
	
	for (int i = 0;i<TOTAL1;i++) 
	{
		texcoords[i*8] = (i*1.0)/(TOTAL1 * 1.0);
		texcoords[i*8 + 1] = 0.0;
		
		texcoords[i*8 + 2] = (i*1.0)/(TOTAL1 * 1.0);
		texcoords[i*8 + 3] = 1.0;
		
		texcoords[i*8 + 4] = ((i+1)*1.0)/(TOTAL1 * 1.0);
		texcoords[i*8 + 5] = 1.0;
		
		texcoords[i*8 + 6] = ((i+1)*1.0)/(TOTAL1 * 1.0);
		texcoords[i*8 + 7] = 0.0;
	}
}
                                 ...
                                 textureTo:(GLuint)textureToView
{
	...
	for (int i = 0;i<TOTAL1;i++)
	{
		...
		glTranslatef(-1+i*2.0/TOTAL1, 0, -4);
