...
@interface SFMenuView () <SFMenuItemDelegate> {
    CGFloat _norSize;
    CGFloat _selSize;
    UIColor *_norColor;
    UIColor *_selColor;
...
@property (nonatomic, weak) SFProgressView *progressView;
@property (nonatomic, weak) SFMenuItem *selItem;
@property (nonatomic, strong) UIColor *bgColor;
@property (nonatomic, strong) NSMutableArray *frames;
@end
// 下划线的高度
static CGFloat const SFProgressHeight = 2.0;
...
- (CGFloat)progressHeight {
    if (_progressHeight == 0.0) {
        _progressHeight = SFProgressHeight;
    }
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    SFMenuItem *currentItem = (SFMenuItem *)[self viewWithTag:tag];
    SFMenuItem *nextItem = (SFMenuItem *)[self viewWithTag:tag+1];
    if (rate == 0.0) {
        rate = 1.0;
        self.selItem.rate = 0;
...
- (void)selectItemAtIndex:(NSInteger)index {
    ...
    SFMenuItem *item = (SFMenuItem *)[self viewWithTag:tag];
    [self.selItem deselectedItemWithoutAnimation];
    self.selItem = item;
    [self.selItem selectedItemWithoutAnimation];
...
- (void)makeStyle {
    switch (self.style) {
        case SFMenuViewStyleLine:
            [self addProgressView];
            break;
        case SFMenuViewStyleFoold:
            [self addFooldViewHollow:NO];
            break;
        case SFMenuViewStyleFooldHollow:
...
- (void)addItems {
    ...
    for (int i = 0; i < self.items.count; i++) {
        ...
        SFMenuItem *item = [[SFMenuItem alloc] initWithFrame:frame];
        item.tag = (i+kTagGap);
        item.delegate = self;
...
- (void)addProgressView {
    SFProgressView *pView = [[SFProgressView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - self.progressHeight, self.scrollView.contentSize.width, self.progressHeight)];
...
- (void)addFooldViewHollow:(BOOL)isHollow {
    SFFooldView *fooldView = [[SFFooldView alloc] initWithFrame:CGRectMake(0, 0, self.scrollView.contentSize.width, self.frame.size.height)];
...
- (void)didPressedMenuItem:(SFMenuItem *)menuItem {
