...
/*
 *  SFPageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (SFPageControllerCachePolicyLoSFemory)，并在一段时间后切换到 High .
    收到多次警告后，会停留在到 SFPageControllerCachePolicyLoSFemory 不再增长
 *
 *  The Default cache policy is No Limit, when recieved memory warning, page controller will switch mode to 'LoSFemory'
    and continue to grow back after a while.
    If recieved too much times, the cache policy will stay at 'LoSFemory' and don't grow back any more.
 */
typedef NS_ENUM(NSUInteger, SFPageControllerCachePolicy){
    SFPageControllerCachePolicyNoLimit   = 0,  // No limit
    SFPageControllerCachePolicyLoSFemory = 1,  // Low Memory but may block when scroll
    SFPageControllerCachePolicyBalanced  = 3,  // Balanced ↑ and ↓
    SFPageControllerCachePolicyHigh      = 5   // High
};

@interface SFPageController : UIViewController

/**
 *  values and keys can set properties when initialize child controlelr (it's KVC)
    see issue #11:https://github.com/wangmchn/SFPageController/issues/11
 ...
 */
@property (nonatomic, assign) SFMenuViewStyle menuViewStyle;
 ...
 */
@property (nonatomic, assign) SFPageControllerCachePolicy cachePolicy;
