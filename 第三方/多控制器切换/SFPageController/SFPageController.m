...
#import "SFPageConst.h"
#import "TableViewController.h"

@interface SFPageController () <SFMenuViewDelegate,UIScrollViewDelegate> {
    CGFloat _viewHeight;
    CGFloat _viewWidth;
    CGFloat _viewX;
    CGFloat _viewY;
...
@property (nonatomic, weak) SFMenuView *menuView;
...
@implementation SFPageController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view, typically from a nib.
    /*
    //在主页使用时，加入以下代码，将此controller设为主视图
    NSMutableArray *viewControllers = [[NSMutableArray alloc] init];
    NSMutableArray *titles = [[NSMutableArray alloc] init];
    for (int i = 0; i < 10; i++) {
        Class vcClass = [TableViewController class];
        NSString *title = [NSString stringWithFormat:@"第%d个table",i];
        [viewControllers addObject:vcClass];
        [titles addObject:title];
    }
    self.title = @"新闻";
    self.pageAnimatable = YES;
    _viewControllerClasses = viewControllers;
    _titles = titles;
    [self setup];
 ...
 SFPageController *pageVC = [self p_defaultController];
 pageVC.title = @"新闻";
 [self.navigationController pushViewController:pageVC animated:YES];
 }
 - (SFPageController *)p_defaultController {
 NSMutableArray *viewControllers = [[NSMutableArray alloc] init];
 NSMutableArray *titles = [[NSMutableArray alloc] init];
 for (int i = 0; i < 10; i++) {
 Class vcClass = [TableViewController class];
 ...
 SFPageController *pageVC = [[SFPageController alloc] initWithViewControllerClasses:viewControllers andTheirTitles:titles];
 ...
 return pageVC;
    ...
    */
    ...
    [self addScrollView];
//    [self addMenuView];
    self.menuViewStyle = SFMenuViewStyleLine;
...
- (void)setCachePolicy:(SFPageControllerCachePolicy)cachePolicy {
...
- (void)postAddToSuperViewNotificationWithIndex:(int)index {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:SFControllerDidAddToSuperViewNotification
                                                        object:info];
}
...
- (void)postFullyDisplayedNotificationWithCurrentIndex:(int)index {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:SFControllerDidFullyDisplayedNotification
                                                        object:info];
}

// 初始化一些参数，在init中调用
- (void)setup {
    _titleSizeSelected  = SFTitleSizeSelected;
    _titleColorSelected = SFTitleColorSelected;
    _titleSizeNormal    = SFTitleSizeNormal;
    _titleColorNormal   = SFTitleColorNormal;
    
    _menuBGColor   = SFMenuBGColor;
    _menuHeight    = SFMenuHeight;
    _menuItemWidth = SFMenuItemWidth;
...
- (void)addMenuView {
    ...
    SFMenuView *menuView = [[SFMenuView alloc] initWithFrame:frame buttonItems:self.titles backgroundColor:self.menuBGColor norSize:self.titleSizeNormal selSize:self.titleSizeSelected norColor:self.titleColorNormal selColor:self.titleColorSelected];
...
- (void)resetMenuView {
    SFMenuView *oldMenuView = self.menuView;
    [self addMenuView];
    [oldMenuView removeFromSuperview];
}

- (void)growCachePolicyAfterMemoryWarning {
    self.cachePolicy = SFPageControllerCachePolicyBalanced;
...
- (void)growCachePolicyToHigh {
    self.cachePolicy = SFPageControllerCachePolicyHigh;
}
...
- (void)didReceiveMemoryWarning {
    ...
    self.cachePolicy = SFPageControllerCachePolicyLoSFemory;
    // 取消正在增长的 cache 操作
...
#pragma mark - SFMenuView Delegate
