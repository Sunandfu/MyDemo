...
@property (nonatomic, copy  ) WSLaunchAdClickBlock adClickBlock;    //广告点击
@property (nonatomic, assign) SkipShowType         showSkipType;
@property (nonatomic,strong ) CAShapeLayer         *shapelayer;
@end

@implementation WSLaunchAD
...
+ (instancetype)initImageWithAttribute:(NSInteger)adDuration showSkipType:(SkipShowType)showSkipType setLaunchAd:(WSSetLaunchAdBlock)setLaunchAd{
    static WSLaunchAD *launchAd = nil;
...
- (void)setWebImageWithURL:(NSString *)strURL options:(WSWebImageOptions)options result:(WSWebImageCompletionBlock)result adClickBlock:(WSLaunchAdClickBlock)adClickBlock{
    ...
    [_launchAdImgView ws_setImageWithURL:[NSURL URLWithString:strURL] placeholderImage:kPlaceholderImage options:options completed:result?result:nil];
}
#pragma mark - 添加广告图
- (void)addAdImgView{
...
+ (void)clearDiskCache{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *path = [WSWebImageDownloader cacheImagePath];
        [fileManager removeItemAtPath:path error:nil];
        [WSWebImageDownloader checkDirectory:path];
    });
}
...
- (UIImageView *)bottomImgView{
    if(!_bottomImgView){
        _bottomImgView = [[UIImageView alloc] initWithFrame:kScreen_Bounds];
