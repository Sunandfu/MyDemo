...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size *3/4)])) {
...
//        self.imageView = [[UIImageView alloc] initWithFrame:CGRectInset(view.bounds, 20, 20)];
...
+ (void)showWithWhiteBack {
    [self dismiss:^{
        [APPDELEGATE.window addSubview:[[self instance] whiteView]];
        [self show];
    }];
}
+ (void)show {
...
- (UIView *)whiteView {
    ...
    if (!self.overlayView) {
        self.overlayView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, APPDELEGATE.window.frame.size.width, APPDELEGATE.window.frame.size.height-64-49)];
