...
@interface SFMoreDetailViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SFMoreDetailViewController
...
- (void)updateFrameWithSize:(CGSize)size{
    ...
    if (size.width > size.height) {
        self.listTableView.frame = CGRectMake(0, 0, fminf(size.width,size.height) * 0.8, CGRectGetHeight(self.menuContentView.frame));
        self.tableView.frame = CGRectMake(0, 20, size.width, size.height-20);
    } else {
        self.listTableView.frame = CGRectMake(0, [SFSafeAreaInsets shareInstance].safeAreaInsets.top, fminf(size.width,size.height) * 0.8, CGRectGetHeight(self.menuContentView.frame)-[SFSafeAreaInsets shareInstance].safeAreaInsets.top);
...
- (void)getWebDataWithIndex:(NSInteger)index{
    ...
    [SFNetWork getXmlDataWithURL:detailModel.postUrl parameters:nil success:^(id data) {
        ...
        if (!xmlError) {
            ONOXMLElement *postsParentElement = [doc firstChildWithXPath:self.bookModel.bookContent];
...
//                imgUrl = [imgUrl stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
                if (imgUrl.length>0) {
                    ...
                    detailModel.postUrl = imgUrl;
                    detailModel.height = 1000/(1420/screenW);
