...
@interface CategoryViewController ()<SFScrollPageViewDelegate>
...
- (void)getClassify{
    NSString *url = [NSString stringWithFormat:@"%@%@",@"https://wap.xsbiquge.com",@"/xclass/0/1.html"];
    [SFNetWork getXmlDataWithURL:url parameters:nil success:^(id data) {
        ...
        ONOXMLElement *postsParentElement= [doc firstChildWithXPath:@"//*[@class=\"sortChannel_nav\"]"]; //寻找该 XPath 代表的 HTML 节点,////*[@id="fj_1"]
        //遍历其子节点,
        [postsParentElement.children enumerateObjectsUsingBlock:^(ONOXMLElement *element, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([[element tag] isEqualToString:@"a"]) {
                [self postWithHtmlStr:element];
            }
        }];
...
-(void)postWithHtmlStr:(ONOXMLElement*)element{
    BookDetailModel *newp = [BookDetailModel new];
    newp.postUrl= [NSString stringWithFormat:@"%@%@",@"https://wap.xsbiquge.com",[element valueForAttribute:@"href"]]; //获取 a 标签的  href 属性
    newp.title= [element stringValue];
...
- (UIViewController<SFScrollPageViewChildVcDelegate> *)childViewController:(UIViewController<SFScrollPageViewChildVcDelegate> *)reuseViewController forIndex:(NSInteger)index {
    ...
    if (!childVc) {
        ...
        BookListViewController *newChildVc = [[BookListViewController alloc] init];
