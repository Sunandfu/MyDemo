...
//    NSLog(@"old ---- %ld current --- %ld", _oldIndex, _currentIndex);
    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollViewSliding)]) {
        [self.delegate scrollViewSliding];
    }
...
- (void)willAppearWithIndex:(NSInteger)index {
    UIViewController<SFScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", (long)index]];
    if (controller) {
...
//    NSLog(@"消失消失:current:---- %ld   old ----- %ld indexpathRow----%ld ", _currentIndex, _oldIndex, indexPath.row);
    ...
    if (!self.forbidTouchToAdjustPosition) {
        if (_currentIndex == indexPath.row) {// 没有滚动完成
            ...
            if (_needManageLifeCycle) {
                UIViewController<SFScrollPageViewChildVcDelegate> *currentVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", (long)_oldIndex]];
                // 开始出现
                [currentVc beginAppearanceTransition:YES animated:NO];
                
                UIViewController<SFScrollPageViewChildVcDelegate> *oldVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", (long)indexPath.row]];
                // 开始消失
