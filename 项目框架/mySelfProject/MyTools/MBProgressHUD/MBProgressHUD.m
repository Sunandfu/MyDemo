...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated andType:(MBProgressHUDNative)type {
    MBProgressHUD *h = (MBProgressHUD *)[view viewWithTag:100000];
    if (h) {
        [h removeFromSuperview];
    }
    
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view andType:type];
	[view addSubview:hud];
	[hud show:animated];
...
- (id)initWithFrame:(CGRect)frame andType:(MBProgressHUDNative)type {
	...
	if (self) {
        ...
        if (type == MBProgressHUDNativeYES)
        {
            self.opacity = 0.0f;
        }
        else
...
- (id)initWithView:(UIView *)view andType:(MBProgressHUDNative)type{
	...
	id me = [self initWithFrame:view.bounds andType:type];
...
- (id)initWithWindow:(UIWindow *)window {
	return [self initWithView:window andType:MBProgressHUDNativeYES];
}
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 60000 // __IPHONE_6_0
...
- (void)updateIndicators:(MBProgressHUDNative)type {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
        ...
        if (type == MBProgressHUDNativeYES)
        {
            ...
            self.indicator = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 30, 30)];
        ...
        {
            UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 96, 132)];
            imageView.animationImages = [[NSArray alloc] initWithObjects:[UIImage imageNamed:@"loading1.png"],[UIImage imageNamed:@"loading2.png"],[UIImage imageNamed:@"loading3.png"],nil];
            imageView.animationDuration = 0.2;
            self.indicator = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 96, 132)];
//            self.indicator.backgroundColor = [UIColor clearColor];
            [self.indicator addSubview:imageView];
            [imageView startAnimating];
        }
...
//    detailsLabelSize = [detailsLabel.text sizeWithFont:detailsLabel.font constrainedToSize:maxSize lineBreakMode:detailsLabel.lineBreakMode];
    ...
    detailsLabelSize = [detailsLabel.text boundingRectWithSize:maxSize options:(NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin) attributes:attributes context:nil].size;
...
- (void)updateUIForKeypath:(NSString *)keyPath {
	if ([keyPath isEqualToString:@"mode"] || [keyPath isEqualToString:@"customView"]) {
        [self updateIndicators:MBProgressHUDNativeYES];
